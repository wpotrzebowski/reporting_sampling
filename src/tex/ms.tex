...
This range can be throught of as a having a prior probability distribution, $p(\mathbf{x})$, where values of $x$ outside of this range have a probability of \num{0}. 
...
\end{equation}
%
Therefore, when Bayesian modelling is performed, the priors and likelihood are of fundamental important to the results that are obtained, the posterior, and therefore any scientific conclusions that are drawn. 
...
In this letter, we outline best practice for reporting the results of Bayesian analysis for neutron and X-ray reflectometry, we hope that this work will engage others to carefully consider how they report this information. 
...
The most common prior probability that is used for parameter is uniform between two values.
The used of a bounded parameter along with some tradiational $\chi^2$-minimisation method and a parameter with a uniform prior and a Bayesian maximum \emph{a-posteriori} approach will lead to the same result. 
...
For example, if the prior is taken from a single complementary measurement that is deinfed as a value with some uncertainty, which represents a normal distribution with a mean and standard deviation, this information can be provided. 
...
The chain is all of the samples investigated in the sampling should be share, although in the case that this chain is very large a subsampled object shared, in which case, the autocorrelation analysis performed should be described (see Section~\ref{sec:posterior} for complete discussion of this).
In order to use such a prior probability in Bayesian analysis, it is necessary that some functional description of the prior is generated, most commonly this will be some kernel density estimation, when this is used it is also necessary to state the structure of the kernel being used. 
...
\begin{figure}
  ...
  \caption{
    A prior probability distribution for a DPPC lipid that could arise from a molecular dynamics simulation (orange histrogram) and a Gaussian kernel density estimation for the probability distribution using a badwidth factor of \num{0.05} (blue line). 
  }
  \label{fig:samples}
\end{figure}
%
...
The former describes our current understanding of the system before we conduct some experiment, while the latter describes the understanding that we gain from the data. 
Therefore, it is necessary that we state how the likelihood for a given model and data is calculated, in particular because although Equation~\ref{equ:likelihood} is a common approach, it assumes a normally distributed uncertainty. 
...
\begin{acknowledgments}
    All authors gratefully acknowledgment the contribution of Rev. T. Bayes (1701-1761). 
\end{acknowledgments}
